...
class ParsingUtilities:
    ...
    def parse_video(file_location: str or bytes or pathlib.Path or io.IOBase) -> dict:
        '''
        Converts video to BytesIO and gets duration and thumbnail.
        '''
        file_location = get_file_bytes(file_location)
        if isinstance(file_location, bytes):
            file_location = io.BytesIO(file_location)
        ...
        proc = subprocess.Popen(
            ['ffmpeg', '-ss', thumbnail_time, '-i', file_location, '-vframes', '1', '-q:v', '2', '-'],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE
        )

        stdout, stderr = proc.communicate()
        thumbnail_bytes = BytesIO(stdout)
        thumbnail = parse_image(thumbnail_bytes)
        size = len(file_location)
        duration_pre = ffmpeg.probe(file_location)["format"]["duration"]
        duration = round(float(duration_pre))
        base64 = ParsingUtilities.read_file_as_base64(final_pre)
