...
class KikClient:
    ...
    def send_video(self, peer_jid: str, file_location):
        ...
        :param peer_jid: The Jabber ID for which to send the message (looks like username_ejs@talk.kik.com)
                         If you don't know the JID of someone, you can also specify a kik username here.
        :param file: The path to the video file OR its bytes OR an IOBase object to send.
        """
        log.info("[+] Sending a video message to '{}'...".format(peer_jid))
        videoRequest = chatting.OutgoingVideo(peer_jid, file_location)
        content.upload_gallery_video(videoRequest, self.kik_node + '@talk.kik.com', self.username, self.password)
        return self._send_xmpp_element(videoRequest)
