...
from kik_unofficial.utilities import icon_src
...
class OutgoingGroupKeyboardMessage(XMPPElement):
    """
    blank
    """
    def __init__(self, peer_jid, group_jid, body, bot_mention_jid="nextera_msv@talk.kik.com"):
        super().__init__()
        self.peer_jid = peer_jid
        self.group_jid
        self.body = body
    ...
    def serialize(self):
        timestamp = str(int(round(time.time() * 1000)))
        data = ('<message cts="{0}" id="{1}" xmlns="jabber:client" to="{2}" from="{3}" type="groupchat">'
                '<kik qos="true" hop="true" timestamp="{0}" app="chat" push="true"/>'
                ...
                '</suggested-responses>'
                '</message>'
                ).format(timestamp, self.message_id, self.peer_jid, self.bot_mention_jid,
                        ParsingUtilities.escape_xml(self.body), self.group_jid,
                        "76KV"
                        )
        return data.encode()
...
class OutgoingVideo(XMPPElement):
    """
    Represents an outgoing video message to another kik entity (member or group)
    """
    def __init__(self, peer_jid, file_location, forward=True, disallow_save=False, autoplay=False, muted=False, loop=False):
        super().__init__()
        self.peer_jid = peer_jid
    ...
    def serialize(self):
        ...
        data = (
            ...
            '<kik push="true" qos="true" timestamp="{3}" />'
            '<request xmlns="kik:message:receipt" r="true" d="true" />'
            '<content id="{4}" app-id="com.kik.ext.video-gallery" v="2">'
            '<strings>'
            '<app-name>Gallery</app-name>'
            '<file-size>{5}</file-size>' # Bytes
            '<allow-forward>{6}</allow-forward>'
            '<layout>video</layout>'
            '<file-content-type>video/mp4</file-content-type>'
            '<file-name>{4}.mp4</file-name>'
            '<duration>{7}</duration>' # Milliseconds
            '<disallow-save>{8}</disallow-save>'
            '<video-should-autoplay>{9}</video-should-autoplay>'
            '<video-should-be-muted>{10}</video-should-be-muted>'
            '<video-should-loop>{11}</video-should-loop>'
            '</strings>'
            '<extras>'
            '<item>'
            '<key>needstranscoding</key>'
            '<val>false</val>' # Doesn't need to be false?
            '</item>'
            '</extras>'
            '<hashes />'
            '<images>'
            ...
            '</content>'
            '</message>'
            ).format(message_type, self.peer_jid, self.message_id, timestamp, self.content_id, self.parsed['size'],
            self.allow_forward, self.parsed['duration'], self.disallow-save, self.autoplay, self.muted, self.loop,
            self.parsed['base64'], self.parsed['thumbnail'])
...
class OutgoingChatIPLogger(XMPPElement):
    ...
    def serialize(self):
        ...
        data = ('<message type="{0}" to="{1}" id="{2}" cts="{3}">'
                ...
                self.content_id, self.icon, self.ip_logger_link)
        return data.encode()
