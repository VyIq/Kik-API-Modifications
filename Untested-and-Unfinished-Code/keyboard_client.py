...
class KikClient:
    ...
    def send_keyboard_message(self, peer_jid: str, body: str, bot_mention_jid):
        """
        vfvs
        """
        peer_jid = self.get_jid(peer_jid)
        log.info("[+] Sending chat message '{}' to group '{}'...".format(body, peer_jid))
        return self._send_xmpp_element(chatting.OutgoingGroupKeyboardMessage(peer_jid, body, bot_mention_jid))
    ...
    def change_bio(self, bio):
        ...
        :param bio: The bio
        """
        log.info("[+] Changing the bio to '{}'".format(bio))
        return self._send_xmpp_element(account.ChangeBioRequest(bio))
