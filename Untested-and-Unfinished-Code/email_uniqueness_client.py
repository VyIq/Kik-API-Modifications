...
class KikClient:
    ...
    def send_fake_system_message(self, peer_jid, sysmsg_body):
        ...
        """
        if self.is_group_jid(peer_jid):
            log.info("[+] Sending '{}' fake system message to group '{}'...".format(peer_jid))
            return self._send_xmpp_element(chatting.OutgoingGIFMessage(peer_jid, sysmsg_body, True))
        else:
            log.info("[+] Sending '{}' fake system message to user '{}'...".format(peer_jid))
            return self._send_xmpp_element(chatting.OutgoingGIFMessage(peer_jid, sysmsg_body, False))

    # Can be sent to a persons direct messages but it won't be visible.
    ...
    def check_email_uniqueness(self, email):
        ...
        Results are returned in the on_email_uniqueness_received() callback
